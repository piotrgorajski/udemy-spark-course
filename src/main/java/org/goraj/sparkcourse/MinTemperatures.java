...
public class MinTemperatures {
    ...
    public static void main(String[] args) {
        ...
        JavaRDD<Tuple3<String, String, Double>> parsedLines = lines.map(MinTemperatures::parseLine);

        // Filter out all but TMIN entries
        JavaRDD<Tuple3<String, String, Double>> minTemps = parsedLines.filter(x -> x._2().equals("TMIN"));

        // Convert to (stationID, temperature)
        JavaPairRDD<String, Double> stationTemps = minTemps.mapToPair(x -> new Tuple2<>(x._1(), x._3()));

        // Reduce by stationID retaining the minimum temperature found
        JavaPairRDD<String, Double> minTempsByStation = stationTemps.reduceByKey(Math::min);

        // Collect, format, and print the results
        List<Tuple2<String, Double>> results = minTempsByStation.collect();

        results.forEach(result -> System.out.printf("%s minimum temperature: %.2f C%n", result._1, result._2));
    }
